...
- `proposal.md`：整体需求文档，定义知识点体系、题库结构、输出范围与优先级
- `qa.md`：需求澄清记录。后续若有新问题，按你的要求继续记录在这里
- `tasks.md`：按章节拆分的任务清单，包含内容条目和质量标准

### 2. 核心学习材料

- `knowledge_base.md`：必备知识点详解，按论文 Task 组织，适合系统补理论
- `interview_questions.md`：模拟面试题库，每题提供保研版、就业版、速答版三种回答

### 3. 面试表达与防守材料

- `elevator_pitch.md`：30 秒、1 分钟、3 分钟项目介绍话术
- `model_comparison.md`：模型选择论证，回答“为什么选这个而不是那个”
- `weakness_defense.md`：弱点与局限性的防守模板，覆盖数据、方法、结果与泛化问题

## 建议使用顺序

如果你要从零开始准备，建议按下面顺序阅读：

1. 先看 `README.md`，建立全局框架
2. 看 `proposal.md` 和 `qa.md`，明确文档定位、目标人群和输出标准
3. 通读 `knowledge_base.md`，补齐四个 Task 的理论与方法
4. 配合 `interview_questions.md` 做高频问答训练
5. 用 `elevator_pitch.md` 打磨开场表达
6. 用 `model_comparison.md` 和 `weakness_defense.md` 应对追问与质疑

如果临近面试，只想高效冲刺，建议顺序改为：

1. `elevator_pitch.md`
2. `interview_questions.md`
3. `weakness_defense.md`
4. `model_comparison.md`
5. 回查 `knowledge_base.md`
...
- 如果后续出现新的澄清问题，我会先提问，并按要求记录到 `qa.md`
